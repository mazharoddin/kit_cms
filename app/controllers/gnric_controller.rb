class GnricController <  ActionController::Base
  ...
  def gnric_layout_in_use
    l = nil

    if self.layout_name_being_used # this gets set if gnric_render is being used
  ...
  def render(name = params[:action], options = {})
    if Preference.get_cached(_sid, 'dont_use_overridable_templates')=='true' || (params[:controller] && params[:controller].starts_with?('admin/'))
      ...
      gnric_render(name, options)
    end
  end

  def gnric_render(name, options = {})
    if options[:partial]
