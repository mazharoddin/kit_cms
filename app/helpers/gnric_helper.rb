module GnricHelper
  ...
  require 'digest/md5'
  # gnric_calendar - see calendar_helper
  ...
  def gnric_javascripts(additional = [])
    op = []
    (additional + javascripts).each do |name|
      next if name.is_blank?
      js = HtmlAsset.fetch(_sid, name.downcase.strip, "js")
      if js
        op << "<script type='text/javascript' src='/kit/js/#{js.gnric_name.downcase}'></script>"
      else
  ...
  def gnric_stylesheets(additional = [])
    op = []
    begin
    s = stylesheets 
    rescue Exception => e
    ...
    (additional + s).each do |name|
      ...
      if sheet
        op << "<link rel='stylesheet' type='text/css' href='/kit/css/#{sheet.gnric_name.downcase}'>"
      else
  ...
  def gnric_pagination(model)
    r = '<div class="gnric_pagination">'
  ...
  def gnric_submit(label, options = {})
    options[:class] ||= ''
...
#  end
  ...
  def _sid
    @gnric_system.get_system_id
  end

  def gnric_default_header
    gnric_render :partial=>"layouts/gnric_header"
  end
