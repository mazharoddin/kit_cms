module GnricModulesHelper

  def gnric_page_list(options={})
    ...
    if options[:template_snippet]
      template = gnric_snippet!(_sid, options[:template_snippet])
    else
  ...
  def gnric_page_terms_by_ids(ids)
    if @page
  ...
  def gnric_page_terms(id) 
    gnric_page_terms_by_ids([id])
  end

  def gnric_term_options(term_id, page_template_id)
  ...
  def gnric_term_parent_list(term_id, page_template_id) # requires a block to which this will yield for each  parent term found, OR returns an array of arrays
    opts = gnric_term_options(term_id, page_template_id)
  ...
  def gnric_term_child_list(term_id, page_template_id, parent_value)  # requires a block to which this will yield for each child term of the given parent
    opts = gnric_term_options(term_id, page_template_id)
    return "term '#{term_id}' not found" unless opts
  ...
  def gnric_asset_src(id, size = :original, missing = "/images/not-found.png")
    src = Asset.asset_url(_sid, id, size)
    return src ? src : missing
  end

  def gnric_asset(id, size = :original, options = {})
    src = Asset.asset_url(_sid, id, size)
    if src 
  ...
  def gnric_asset_tagged_src(tag, size = :original, missing = "/images/not-found.png")
    src = Asset.asset_tagged_url(_sid, tag, size)
    return src ? src : missing
  end

  def gnric_asset_tagged(tag, size = :original, options = {})
    src = Asset.asset_tagged_url(_sid, tag, size)
    if src 
      return image_tag src, options
    else
  ...
  def gnric_forums_threads_im_on(count=5, options={})
    mod = can?(:moderate, ForumController)
   
    if current_user
      threads = TopicThread.im_on(current_user, count, mod)
      gnric_render :partial=>"forum/thread_list",:locals=>{:threads=>threads, :options=>options} 
    else
     ""
    end 
  end

  def gnric_forums_favourites(count=5, options={}) # also 'threads I'm watching'
  ...
  def gnric_forums_recent_posts(count=5, options = {})
    mod = can?(:moderate, ForumController)
  ...
  def gnric_forums_recent_threads_from_topic_id(topic_id, count = 5, options = {})
  ...
  def gnric_forums_recent_threads_from_category_id(category_id, count = 5, options = {})
  ...
  def gnric_forums_topics
    topics = Topic.sys(_sid).where(["topics.read_access_level <= ?", current_user ? current_user.forum_level : 0]).where(:is_visible=>1).all

    gnric_render :partial=>"forum/topic_list", :locals=>{:topics => topics, :category=>category, :show_headings=>false, :show_mod=>false, :show_meta=>false, :paginate=>false}
  end

  
  def gnric_forums_topics_from_category_id(category_id)
  ...
  def gnric_calendar_entry_add(calendar_id, options = {})
    module_gnric_calendar_entry_add(calendar_id, options)
  end

  def gnric_calendar_entry(id, options = {})
    module_gnric_calendar_entry(id, options)
  end

  def gnric_calendar(calendar_id, options = {})
    module_gnric_calendar(calendar_id, options)
  end
  ...
  def gnric_menu(id) 
    ...
    html = (gnric_render :partial=>"menu/render", :locals=>{:name=>id, :menu=>menu}).html_safe
  ...
  def gnric_dated_snippet_line(id)
    begin
      ...
      logger.debug("Error with gnric_dated_snippet_line")
      logger.debug(e)
  ...
  def gnric_current_year
    Time.now.strftime("%Y")
  end  

  def gnric_current_month(format = '%b')
    Time.now.strftime(format)
  end

  def gnric_block_without_instance(id, options = {})
  ...
  def gnric_template_block(instance_id, block_id, defaults = {})
    gnric_editable_block(instance_id, block_id, defaults)
  end

  def gnric_editable_block(instance_id, block_id, defaults = {})
  ...
  def gnric_map(location, title="", height = 400, width = 400)
    gnric_render :partial=>"utility/map", :locals=>{height: height, width: width, location: location, title:title}
  end

  def gnric_snippet!(sid, id)
  ...
  def gnric_snippet(id, default = nil, sid = nil)
  ...
  def gnric_users_email
    current_user ? current_user.email : ""
  end

  def gnric_search
    gnric_render :partial=>"pages/search_form", :locals=>{dashboard:false}
  end

  def gnric_page_field(page_id, field_name, is_template = false)
  ...
  def gnric_form(id, show_title = false, show_body = false, no_permissions="You need to sign in to see this")
    begin
    if id.is_number?
    ...
    if form
      if form.public_creatable || (current_user && form.user_creatable)
        gnric_render(:partial=>"form/show", :locals=>{:form=>form, :show_title=>show_title, :show_body=>show_body, :submission=>nil}) 
      else
        no_permissions
      end
    else
  ...
  def gnric_form_edit_submission(id, submission_id, show_title = false, show_body = false, no_permissions="You need to sign in to see this")
    if id.is_number?
      form = Form.where(:id=>id).sys(_sid).first
    else
      form = Form.where(:url=>id).sys(_sid).first
    ...
    if sub && form
      if sub.can_edit?(current_user)
        gnric_render(:partial=>"form/show", :locals=>{:form=>form, :show_title=>show_title, :show_body=>show_body, :submission=>sub}) 
      else
        no_permissions
      end
    else
  ...
  def gnric_slideshow(id, options)
